...
public class OptionPanel {
	...
	private static class NumberProperty implements PropertyControl,
			PropertyBean {
		String description;
		JSlider slider;
		String label;
		 ...
		 * @param label
		 */
		public NumberProperty(String description, String label, int min,
				int max, int step) {
		...
		public void layout(DefaultFormBuilder builder) {
			...
			JLabel label = builder.append(OptionPanel.getText(getLabel()),
					spinner);
			label.setToolTipText(OptionPanel.getText(getDescription()));
		}
	...
	// CLOUDS
	public static class ColorProperty extends JButton implements
		...
		public void layout(DefaultFormBuilder builder) {
			...
			// FIXME: Translate me!
			JMenuItem item = new JMenuItem(fmMain
					.getResourceString("OptionPanel.ColorProperty.ResetColor"));
	...
	// CLOUDS
	public static class KeyProperty extends JButton implements PropertyControl,
			PropertyBean {
		String description;
	...
	private Vector getControls() {
		...
		"language.tooltip", FreeMind.RESOURCE_LANGUAGE, new String[] {
				...
				"ja", "kr", "lt", "nl", "no", "pl", "pt_BR", "pt_PT", "ru",
				"sl", "zh", "zh_CN" })); // automatic
		...
		// "modes_since_0_8_0")); //
		//freemind.modes.browsemode.BrowseMode,Browse,freemind.modes.mindmapmode
		// .MindMapMode,MindMap,freemind.modes.filemode.FileMode,File
		//
		...
		controls.add(new NextLineProperty());
		controls.add(new StringProperty(null, "userproperties")); // user.
																	// properties
		...
		"browsemode_initial_map.tooltip", "browsemode_initial_map")); // ./doc/
																		// freemind
																		// .mm
		controls.add(new NextLineProperty());
		...
		"standardselectednodecolor.tooltip",
				FreeMind.RESOURCES_SELECTED_NODE_COLOR, "#D2D2D2")); // #D2D2D2

		controls.add(new ColorProperty(

		"standardedgecolor.tooltip", FreeMind.RESOURCES_EDGE_COLOR, "#808080")); // #
																					// 808080

		controls.add(new ColorProperty(

		"standardlinkcolor.tooltip", FreeMind.RESOURCES_LINK_COLOR, "#b0b0b0")); // #
																					// b0b0b0

		controls.add(new ColorProperty(

		"standardbackgroundcolor.tooltip", FreeMind.RESOURCES_BACKGROUND_COLOR,
				"#ffffff")); // #ffffff

		// CLOUDS
		colorPropertyHook(controls);
		...
		"standardedgestyle.tooltip", FreeMind.RESOURCES_EDGE_STYLE,
				new String[] { "bezier", "linear" })); // bezier
		...
		// CLOUDS
		cloudPanelHook(controls); // control
		// shift
		// B
	...
	private void colorPropertyHook(Vector controls) {
		// controls
		// .add(new ColorProperty(
		//
		// "standardcloudcolor.tooltip", FreeMind.RESOURCES_CLOUD_COLOR,
		// "#f0f0f0")); // #f0f0f0
	}

	private void cloudPanelHook(Vector controls) {
		// controls
		// .add(new ColorProperty(
		//
		// "standardcloudcolor.tooltip", FreeMind.RESOURCES_CLOUD_COLOR,
		// "#f0f0f0")); // #f0f0f0
		// controls
		// .add(new KeyProperty(frame, null, "keystroke_node_toggle_cloud"));
	}

	public void closeWindow() {
		try {
			OptionPanelWindowConfigurationStorage storage = fmMain
					.getController().getActionXmlFactory()
