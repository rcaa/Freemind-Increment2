...
public class MapView extends JPanel implements Printable {
	...
	private class DelayedScroller extends ComponentAdapter {
		...
		public void scrollNodeToVisible(MapView map, NodeView node,
				int extraWidth) {
			deactivate();
			m_map = map;
			m_node = node;
	...
	public MapView(MindMap model, Controller controller) {
		...
		disableMoveCursor = Tools.safeEquals(controller
				.getProperty("disable_cursor_move_paper"), "true");
	}

	public void initRoot() {
		rootView = NodeView.newNodeView((MindMapNode) getModel().getRoot(),
				this);
		rootView.insert();
	...
	public void centerNode(NodeView node) {
		...
		Rectangle rect = new Rectangle(node.getLocation().x
				+ (node.getPreferredSize().width / 2) - (d.width / 2), node
				.getLocation().y
				+ (node.getPreferredSize().height / 2) - (d.height / 2),
				d.width, d.height);
	...
	public void scrollNodeToVisible(NodeView node, int extraWidth) {
		...
		if (node != null) {
			...
			scrollRectToVisible(new Rectangle(x - HORIZ_SPACE, node
					.getLocation().y
					- VERT_SPACE, width + HORIZ_SPACE2, node.getSize().height
					+ VERT_SPACE2));
			mapViewport.setScrollMode(viewPortScrollMode);
			// (PN) scrollRectToVisible( new Rectangle(xLeft,
			// node.getLocation().y - VERT_SPACE,
	...
	public void scrollBy(int x, int y, boolean repaint) {
		...
		if (mapViewport != null) {
			...
			double maxX = getSize().getWidth()
					- mapViewport.getExtentSize().getWidth(); // getView()
			// gets
			// viewed area -
			// JPanel
	...
	private NodeView getNeighbour(int directionCode) {
		...
		case KeyEvent.VK_LEFT:
			setSiblingMaxLevel(oldSelected.getModel().getNodeLevel()); // for
			// case
			// of
			// return
			if (oldSelected.isRoot()) {
				...
				if (left.size() == 0)
					return null;
				newSelected = oldSelected.getModel().getPreferredChild()
						.getViewer();
				...
				if (oldSelected.getModel().isFolded()) { // If folded in the
					// direction, unfold
					getController().getModeController().setFolded(
							oldSelected.getModel(), false);
					return null;
				}
	...
	private void extendSelectionWithKeyMove(NodeView newlySelectedNodeView,
		...
		if (e.isShiftDown()) {
			...
			int deltaY = newlySelectedNodeView.getY()
					- shiftSelectionOrigin.getY();
	...
	/**
	 * @param iterativeLevel
	 *            describes the n-th nested cloud that is to be painted.
	 */
	// CLOUDS
	// public void paintClouds(NodeView source, Graphics graphics) {
			...
			if (LinkAlreadyVisited.add(ref)) {
				...
				if (ref instanceof MindMapArrowLink) {
					ArrowLinkView arrowLink = new ArrowLinkView(
							...
							ref.getSource().getViewer(), ref.getTarget()
									.getViewer());
	...
	private void _endPrinting() {
		if (isPreparedForPrinting == false) {
			...
			setZoom(getZoom());
			fitToPage = Tools.safeEquals(controller.getProperty("fit_to_page"),
					"true");
		}
	}
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) {
		...
		graphics2D.translate(-boundingRectangle.getX(), -boundingRectangle
				.getY());
	...
	public Rectangle getInnerBounds(NodeView source) {
		...
		if (source.getModel().isRoot()) {
			for (int i = 0; i < ArrowLinkViews.size(); ++i) {
				ArrowLinkView arrowView = (ArrowLinkView) ArrowLinkViews.get(i);
				Rectangle arrowViewBigBounds = arrowView.arrowLinkCurve
						.getBounds();
				if (!innerBounds.contains(arrowViewBigBounds)) {
					Rectangle arrowViewBounds = PathBBox.getBBox(
							arrowView.arrowLinkCurve).getBounds();
	...
	private void addHeightOfTheCloud(NodeView source, Rectangle innerBounds) {
		// CLOUDS
		// int additionalCloudHeigth = (source.getAdditionalCloudHeigth() + 1) /
		// 2;
	...
	private class MapModelHandler implements TreeModelListener {
		public void treeNodesChanged(TreeModelEvent e) {
			...
			try {
				...
				nodeView = ((MindMapNode) e.getTreePath()
						.getLastPathComponent()).getViewer();
			}
		...
		public void treeNodesInserted(TreeModelEvent e) {
			...
			MindMapNode parent = (MindMapNode) e.getTreePath()
			...
			if (child != null) { // if a node is folded, child is null.
				...
				// if(node.isFolded()) {
				// logger.info(
				// "Found folded node in path of a new children to be inserted."
				// );
				// return;
				// }
				// }
			...
			getMindMapLayout().updateTreeHeightsAndRelativeYOfAncestors(
					parentView);
		...
		public void treeNodesRemoved(TreeModelEvent e) {
			// This is called once for each removed node, with just one child
			// (the removed)
			NodeView node = ((MindMapNode) e.getChildren()[0]).getViewer();
			// Take care not to remove root in Controller
			NodeView parent = ((MindMapNode) e.getTreePath()
					.getLastPathComponent()).getViewer();
			if (node == null) {
				System.err
						.println("MapView.treeNodesRemoved tried to delete null as node.");
				return;
			}
		...
		/**
		 * Assures that every and at least one selected node is visible. If this
		 * is not the case, the node is deselected. If every selected node is
		 * hidden, the first visible part in the path of the selected node to
		...
		public void treeStructureChanged(TreeModelEvent e) {
			...
			MindMapNode subtreeRoot = (MindMapNode) e.getTreePath()
					.getLastPathComponent();
			...
			if (subtreeRoot == null || subtreeRoot.getViewer() == null) {
				...
				if (subtreeRoot.isRoot()) {
					nodeView = NodeView.newNodeView(getRoot().getModel(),
							getMap());
					rootView = nodeView;
				} else {
				...
				getMindMapLayout()
						.updateTreeHeightsAndRelativeYOfDescendantsAndAncestors(
								subtreeRoot.getViewer());
				// the layout function will be called later by AWT framework
				// itself
				// comment it out
			...
			// Warning, the old views still exist, because JVM has not deleted
			...
			while (focussedNode.getParentNode() != null
					&& focussedNode.getViewer() == null) {
	...
	public int getZoomedFoldingSymbolHalfWidth() {
		if (FOLDING_SYMBOL_WIDTH == -1) {
			FOLDING_SYMBOL_WIDTH = getController().getIntProperty(
