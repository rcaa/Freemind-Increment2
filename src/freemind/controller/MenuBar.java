...
public class MenuBar extends JMenuBar {
	...
	// changed to public
	public StructuredMenuHolder menuHolder;
	...
	public void updateMenus() {
		...
		// format menu
		formatmenu = menuHolder.addMenu(new JMenu(c
				.getResourceString("menu_format")), FORMAT_MENU + ".");
		...
		// Modesmenu
		JMenu modesmenu = menuHolder.addMenu(new JMenu(c
				.getResourceString("modes")), MODES_MENU + ".");
	...
	private void updateModeMenu() {
		...
		for (ListIterator i = keys.listIterator(); i.hasNext();) {
			...
			JRadioButtonMenuItem newItem = (JRadioButtonMenuItem) menuHolder
					.addMenuItem(new JRadioButtonMenuItem(key), MODES_MENU
							+ key);
			group.add(newItem);
			if (c.getMode() != null) {
			...
			String keystroke = c.getFrame()
					.getProperty("keystroke_mode_" + key);
			if (keystroke != null) {
	...
	private void updateMapsMenu(StructuredMenuHolder holder, String basicKey) {
		...
		List keys = new LinkedList(c.getMapModuleManager().getMapModules()
				.keySet());
		...
		for (ListIterator i = keys.listIterator(); i.hasNext();) {
			...
			if (c.getMapModuleManager().getMapModule() != null) {
				if (key.equals(c.getMapModuleManager().getMapModule()
						.toString())) {
	...
	private void updateFileMenu() {
		...
		JMenuItem print = menuHolder.addAction(c.print, FILE_MENU
				+ "print/print");
		print.setAccelerator(KeyStroke.getKeyStroke(c.getFrame().getProperty(
				"keystroke_print")));

		JMenuItem close = menuHolder.addAction(c.close, FILE_MENU
				+ "close/close");
		close.setAccelerator(KeyStroke.getKeyStroke(c.getFrame().getProperty(
				"keystroke_close")));

		JMenuItem quit = menuHolder.addAction(c.quit, FILE_MENU + "quit/quit");
		quit.setAccelerator(KeyStroke.getKeyStroke(c.getFrame().getProperty(
				"keystroke_quit")));
		...
		for (ListIterator it = lst.listIterator(); it.hasNext();) {
			...
			if (firstElement) {
				...
				item.setAccelerator(KeyStroke.getKeyStroke(c.getFrame()
						.getProperty("keystroke_open_first_in_history")));
			}
			item.addActionListener(lastOpenedActionListener);

			menuHolder.addMenuItem(item, FILE_MENU + "last/"
					+ (key.replace('/', '_')));
		}
	}
	...
	private void updateEditMenu() {
		...
		// ICON AOP
		// JMenuItem toggleLeftToolbar =
		// menuHolder.addAction(c.toggleLeftToolbar,
		...
		JMenuItem zoomIn = menuHolder.addAction(c.zoomIn, VIEW_MENU
				+ "zoom/zoomIn");
		zoomIn.setAccelerator(KeyStroke.getKeyStroke(c.getFrame().getProperty(
				"keystroke_zoom_in")));

		JMenuItem zoomOut = menuHolder.addAction(c.zoomOut, VIEW_MENU
				+ "zoom/zoomOut");
		zoomOut.setAccelerator(KeyStroke.getKeyStroke(c.getFrame().getProperty(
				"keystroke_zoom_out")));
		...
		moveToRoot.setAccelerator(KeyStroke.getKeyStroke(c.getFrame()
				.getProperty("keystroke_moveToRoot")));
		...
		previousMap.setAccelerator(KeyStroke.getKeyStroke(c.getFrame()
				.getProperty("keystroke_previousMap")));

		JMenuItem nextMap = menuHolder.addAction(c.navigationNextMap,
				MINDMAP_MENU + "navigate/navigationNextMap");
		nextMap.setAccelerator(KeyStroke.getKeyStroke(c.getFrame().getProperty(
				"keystroke_nextMap")));

		// JMenu preferences = menuHolder.addMenu(new
		// JMenu(c.getResourceString("preferences")),
		// EXTRAS_MENU+"last/options/.");
		JMenuItem prefDialog = menuHolder.addAction(c.propertyAction,
				EXTRAS_MENU + "last/option_dialog");
		prefDialog.setAccelerator(KeyStroke.getKeyStroke(c.getFrame()
				.getProperty("keystroke_option_dialog")));
	...
	private void addOptionSet(Action action, String[] textIDs, JMenu menu,
		...
		for (int optionIdx = 0; optionIdx < textIDs.length; optionIdx++) {
			...
			// keystroke present?
			String keystroke = c.getFrame().getProperty(
